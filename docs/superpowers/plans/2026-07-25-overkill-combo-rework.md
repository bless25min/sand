...
interface CommandDraft { cardIds: readonly string[] }
interface CompiledCommand { cardIds: readonly string[]; diagnostics: readonly string[] }
interface ComboContent { cards: CardCatalog; rules: RuleCatalog; builds: readonly BuildDefinition[] }
interface CompiledBuild { cardIds: readonly string[]; ruleIds: readonly string[] }
interface TriggerQueueInput { battle: GuildBattleState; command: CompiledCommand; rules: RuleCatalog }
interface TriggerQueueResult { battle: GuildBattleState; events: readonly ComboEvent[]; infinite: boolean }
interface HuntRewardInput { profile: GuildProfile; battle: GuildBattleState; hunt: HuntDefinition }
```
...
- [ ] Write tests for tag prerequisites, causal event order, no universal cost, 0.25x enemy pressure,
  full resolution after lethal damage, and deterministic identical inputs.
- [ ] Run `pnpm vitest run packages/simulation-core/src/guild-rpg/combo/combo.test.ts`; verify RED.
- [ ] Add the minimal contracts, 12 cards, compiler, resolver, pressure step, reducer actions, composer,
  and playback required by those tests and the mobile interaction contract.
- [ ] Re-run the combo and guild-rpg component/reducer tests; verify GREEN.
- [ ] Perform one Task 1 review, fix findings, run the affected tests, and commit the batch.
