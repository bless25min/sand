...
- [ ] Write failing tests for empty and wrapping indexes, paged inventory, primary slot, selected
...
- [ ] Implement the two pure helpers and semantic deck shell only; do not wire screen actions yet.
- [ ] Run the same tests and keep screen-contract assertions failing while model and deck assertions
  pass.
...
- [ ] Make the Guild contract green with focused quest, party, and two-item inventory paging.
  Required start, leader, item selection, target adventurer, and equip actions use the deck.
- [ ] Make the Battle contract green. Skills retain fixed positions while disabled; target selection
  returns to Skill; ally-target skills request a recipient inside Target; Auto and speed use Tactics.
- [ ] Make the Reward contract green. Focus one item, cycle adventurer, equip or keep, confirm sell
  twice, and return after all items resolve.
- [ ] Run the two affected tests plus
...
- [ ] Import the deck stylesheet and add mobile-only focused content. Use `100dvh`,
...
- [ ] Keep the existing desktop grids and command controls above 800 pixels.
- [ ] Run one consolidated `pnpm check`; fix only batch-related findings and run one focused
  follow-up for any fix.
- [ ] Browser-test the complete loop at 375 by 812 and 390 by 844: start, select target, attack,
  Auto, 2x, reward decisions, return, inventory equip, reload, and no console errors or overflow.
- [ ] Perform one consolidated diff review against the spec, then commit the implementation once.
