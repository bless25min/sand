...
describe('Cocos production client project', () => {
  ...
  it('keeps combat information readable and makes each relay visibly stronger', async () => {
    ...
    expect(skillDock).toContain('compactSkillName(state.skill.name)');
    expect(skillDock).not.toContain('addText(name, state.skill.name');
    expect(skillDock).toContain('總傷');
    expect(skillDock).not.toContain('MOUSE_UP');
    expect(skillDock).not.toContain('段・追擊');
    ...
    expect(actorAnimator).toContain('windUp(');
    expect(actorAnimator).toContain('strike(');
    expect(actorAnimator).toContain('react(');
    expect(actorAnimator).toContain('recover(');
    ...
    expect(vfxDirector).toContain('routePoints(');
    expect(vfxDirector).toContain('drawProjectile(');
    expect(vfxDirector).toContain('UIOpacity');
    expect(damageNumbers).toContain('node.setParent(target)');
    expect(damageNumbers).not.toContain('×');
    ...
    expect(playback).toContain('createCinematicBeatPlan');
    expect(playback).toContain('onEventVisible');
    expect(playback).toContain('play: async (beat: PresentationBeat, beatSignal: AbortSignal)');
    expect(playback).toContain('Array.from(heldPoses.values())');
    expect(playback).not.toContain('[...heldPoses.values()]');
