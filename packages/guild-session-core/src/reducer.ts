...
import type { GuildPreferences, MotionPreference, TutorialState } from './preferences';
import { reduceHuntResult } from './hunt-result';
import { createSkillEngineContent } from './skill-content';

export type GuildPage = 'quest' | 'party' | 'skills' | 'equipment';
export type SkillWorkspace = 'loadout' | 'fusion';
...
export const GUILD_RPG_ACTION_TYPES = [
  'NAVIGATE',
  ...
  'SET_MOTION',
  'SET_TUTORIAL',
] as const satisfies readonly GuildRpgAction['type'][];
...
export function reduceGuildSession(state: GuildRpgState, action: GuildRpgAction): GuildRpgState {
  ...
  if (action.type === 'DISMANTLE_SKILL' && state.screen === 'guild') {
    ...
    const equippedMember = state.profile.party.find(({ skillIds }) => skillIds.includes(fused.id));
    if (equippedMember) {
      return {
        ...state,
        message: `${fused.name}仍由${heroName(equippedMember.definitionId)}裝備；請先從角色技能格換下再拆解。`,
      };
    }
