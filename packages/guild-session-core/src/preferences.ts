...
export const GUILD_PREFERENCES_KEY = 'expedition:guild-rpg:preferences:v1';
...
export function loadGuildPreferences(
  storage: Pick<GuildSavePort, 'getItem'>,
  hasProfile: boolean,
): GuildPreferences {
  return parseGuildPreferences(storage.getItem(GUILD_PREFERENCES_KEY), hasProfile);
}
...
export function storeGuildPreferences(
  ...
  storage.setItem(GUILD_PREFERENCES_KEY, JSON.stringify(preferences));
}
